...
**Haiku (60%) + Sonnet (40%).** Basic characterization tests (input/output pairs, return value checks) can be generated by Haiku. Adversarial test vectors (encoding edge cases, binary protocol variants, error condition coverage) benefit from Sonnet's ability to reason about what could go wrong.
